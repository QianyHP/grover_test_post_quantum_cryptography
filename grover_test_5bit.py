...
def build_manual_oracle_5bit(target_state):
    """构造适用于 5 比特的 Oracle，相位反转指定目标态。"""
    oracle = QuantumCircuit(5)
    for i, bit in enumerate(reversed(target_state)):
        if bit == '0':
            oracle.x(i)
    oracle.h(4)
    oracle.mcx([0, 1, 2, 3], 4)  # 控制前4位，翻转第5位
    oracle.h(4)
    for i, bit in enumerate(reversed(target_state)):
        if bit == '0':
            oracle.x(i)
...
def adaptive_grover_search_manual_oracle(target_state="10101", max_iter=6, threshold=0.975,
                                         shots=1024, optimization_level=3, backend_name='qasm_simulator'):
    target_bitstr = logic_state_to_qiskit_bitstr(target_state)
    oracle_gate = build_manual_oracle_5bit(target_state)
    oracle_circuit = QuantumCircuit(5)
    oracle_circuit.append(oracle_gate, [0, 1, 2, 3, 4])
...
def plot_results(experimental_probs, theoretical=True):
    ...
    if theoretical:
        theta = np.arcsin(1 / np.sqrt(32))  # N = 2^5
...
# 保存为 PDF
    circuit_drawer(optimized_circuit, output='mpl', filename="grover_5bit_circuit.pdf", style='iqp')
