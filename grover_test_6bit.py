...
def build_manual_oracle_6bit(target_state):
    """构造适用于 6 比特的最优 Oracle，相位反转指定目标态。"""
    oracle = QuantumCircuit(6)
    for i, bit in enumerate(reversed(target_state)):
        if bit == '0':
            oracle.x(i)
    oracle.h(5)
    oracle.mcx([0, 1, 2, 3, 4], 5)  # ✅ 推荐使用 mcx，替代 mct
    oracle.h(5)
    for i, bit in enumerate(reversed(target_state)):
        if bit == '0':
            oracle.x(i)
...
def adaptive_grover_search_manual_oracle(target_state="101011", max_iter=6, threshold=0.997,
                                         shots=1024, optimization_level=3, backend_name='qasm_simulator'):
    target_bitstr = logic_state_to_qiskit_bitstr(target_state)
    oracle_gate = build_manual_oracle_6bit(target_state)
    oracle_circuit = QuantumCircuit(6)
    oracle_circuit.append(oracle_gate, [0, 1, 2, 3, 4, 5])
...
def plot_results(experimental_probs, theoretical=True):
    ...
    if theoretical:
        theta = np.arcsin(1 / np.sqrt(64))  # N = 2^6
...
# 保存为 PDF
    circuit_drawer(optimized_circuit, output='mpl', filename="grover_6bit_circuit.pdf", style='iqp')
